...
import json
from geometric_sym_finder import get_symm
...
if __name__ == "__main__":
    ...
    for idx, obj in tqdm(enumerate(objs), total=len(objs), desc="Processing Objects"):
        ...
        obj_symmetries = get_symm(mesh, h_threshold=0.05, angle_steps=6, result_matrix=True)
        symmetries_path = os.path.join(symmetries_output_dir, f"{idx:06d}.json")
        # save_symmetries(obj_symmetries, symmetries_path)
        with open(symmetries_path, 'w') as json_file:
            # json_file.write(str(obj_symmetries))
            json.dump(obj_symmetries, json_file, indent=4)
